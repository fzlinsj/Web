...
namespace Infrastructure
{
    ...
    public class LogUtil
    {
        ...
        /// 初期化日志操作的一些参数
        /// </summary>
        private static void Init(AppSetting appSettings)
        {
            //读取配置文件
            if (!_IsLoad)
            {
                ...
                //获取日志文件路径
                if (appSettings.FILE_PATH_LOG== null)
                {
                    _LogFilePath = rootPath + "\\log";//ConfigurationManager.AppSettings[FILE_PATH_LOG];
                }
                else
                {
                    _LogFilePath = appSettings.FILE_PATH_LOG;
                }


                //日志备份路径
                if (appSettings.FILE_PATH_LOG_BAK == null)
                {
                    _LogFilePathBak = rootPath + "\\log\\bak";//ConfigurationManager.AppSettings[FILE_PATH_LOG_BAK];
                }
                else
                {
                    _LogFilePathBak = appSettings.FILE_PATH_LOG_BAK;
                }



                //日志文件的最大长度
                _FileMaxSizeLog = appSettings.FILE_MAX_SIZE_LOG;


                //系统日志文件名
                _LogFileNameSystem = appSettings.LOG_FILE_NAME_SYSTEM;
                //登陆日志文件名
                _LogFileNameLogin = appSettings.LOG_FILE_NAME_LOGIN;
                _IsLoad = true;
            }
        }
        ...
        private static void WriteLogToFile(LogInfo log, AppSetting setting)
        {
            //日志文件名
            ...
            //写入日志
            WriteTextToFile(msg, fileName, setting);
        }
        ...
        private static string EditLoginLogInfo(LogInfo log)
        {
            ...
            //登录IP
            sb.Append(log.IpAddress + ConstUtil.TAB);
            //浏览器类型
            sb.Append(log.UserAgent + ConstUtil.TAB);
            //登录类型
        ...
        private static void WriteTextToFile(string msg, string fileName, AppSetting setting)
        {
            //读取配置文件设置
            Init(setting);
            //日志文件目录未创建时，创建文件夹
        ...
        /// 
        /// </remarks>
        public static void WriteLog(LogInfo log, AppSetting setting)
        {
            //操作日志
